...
def test_search_memory_by_type(local_memory_store):
    # Arrange
    local_memory_store.write_memory(MemoryRecord(type="finding_snapshot", content={"f": 1}, created_at=datetime.now()))
    local_memory_store.write_memory(MemoryRecord(type="plan_snapshot", content={"p": 1}, created_at=datetime.now()))
    
    # Act
    results = local_memory_store.search_memory(MemorySearchRequest(type="finding_snapshot"))
...
def test_get_timeline(local_memory_store):
    # Arrange
    local_memory_store.write_memory(MemoryRecord(type="reflection", content={"r": 1}, created_at=datetime.now()))
    local_memory_store.write_memory(MemoryRecord(type="reflection", content={"r": 2}, created_at=datetime.now()))
